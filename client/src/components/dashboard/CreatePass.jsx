...
export default function CreatePassword() {
  ...
  const submitPassword = async () => {
    ...
    try {
      res = await api.post("/api/user/createPassword", {
        name: passwordName,
        password: password,
        chosenCategory: createNewCategory ? newCategoryName : chosenCategory,
        categoryColor: categoryColor,
      });
      if (createNewCategory) {
        uiAddCategories({
          category_name: createNewCategory ? newCategoryName : chosenCategory,
  ...
  return (
    <div className="flex flex-col w-full h-4/5 items-center justify-center bg-gray-200 dark:bg-slate-700 shadow-lg p-10 font-Roboto max-sm:overflow-y-auto">
      <div className="w-full h-full bg-gray-300 dark:bg-slate-800 p-5 shadow-xl">
        <h1 className="font-bold underline text-xl">Create Password</h1>
        <h1 className={`${isError ? "text-red-500" : "text-green-500"} mb-5`}>
          {" "}
          {message === "" ? <br /> : message}
        </h1>
        <div className="flex flex-row w-full mb-10 rounded-2xl shadow-xl">
          <div className="flex justify-center items-center font-bold text-gray-600 bg-blue-300 dark:bg-orange-300 h-10 p-2 border-2 border-blue-400 dark:border-orange-300 rounded-l-2xl">
            Name:
          </div>
          <input
            value={passwordName}
            onChange={(e) => setPasswordName(e.target.value)}
            className="w-full bg-transparent border-2 rounded-r-2xl px-2  border-blue-600 dark:border-orange-600 focus:outline-none focus:border-4"
        ...
        <div className="flex flex-row justify-center items-center gap-4 w-full">
          <button
            ...
            className="rounded-md py-3 px-10 bg-orange-600 text-white font-semibold hover:bg-orange-500 hover:scale-105 shadow-lg "
          >
            Create Password
          </button>
          <button
            onClick={clearForm}
            title="restart"
            className="rounded-full hover:bg-gray-300 dark:hover:bg-slate-700 p-2"
          >
            <RestartAltIcon />
          </button>
        </div>
