...
const Dashboard = ({ errorHandling }) => {
    ...
    const categories = [
        {name: "Create New Password", shortName: "Create", selected: true, icon: <CreateIcon/>},
        {name: "See All Passwords", shortName:"All", selected: false, icon: <ListIcon/>},
        {name: "See Categories", shortName: "Categories", selected: false, icon: <CategoryIcon/>}
    ]
    ...
    if (isLoading) return <div className="flex items-center justify-center font-semibold text-3xl"><p>Loading...</p></div>;

    return (
        <div className="flex flex-row max-sm:flex-col max-sm:justify-start max-sm:mt-20 justify-center items-center w-screen h-screen dark:bg-slate-700 dark:text-white p-5 space-x-5 max-sm:space-x-0 overflow-hidden">
